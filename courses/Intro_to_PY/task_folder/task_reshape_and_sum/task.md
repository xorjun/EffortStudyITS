...
## Axes
In NumPy arrays, axes refer to the dimensions along which the elements of the array are indexed. NumPy arrays can have one or more axes, and each axis is identified by a non-negative integer. This is particularly important for operations that can be performed row- or column-wise, like the `sum` function. For example:
...
You can reshape NumPy arrays by rearranging the elements along different axes. This is often used for preparing data for specific operations or calculations. For example:
