...
def box_plot_differences():
#!prefix!#
    diff_control = np.array(post_test_control) - np.array(pre_test_control) 
    diff_intervention = np.array(post_test_intervention) - np.array(pre_test_intervention) 
    ...
    plt.ylabel('difference post - pre')
    plt.show()
