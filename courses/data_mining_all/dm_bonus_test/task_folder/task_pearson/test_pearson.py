...
def test_pearson():
    ...
    assert type(corr) in [float, np.float32, np.float16, np.float64], f"The return type of your correlation function should be a single floating point number, not {type(corr)}"

    #Self Correlation
    corr = pearson_correlation(x,x)
    assert np.abs(corr - 1) < 0.001, f"The Correlation of an array with itself should be one, not {corr}"

    #Correlation of scaled varrays
    corr = pearson_correlation(x, y)
    assert np.abs(corr - 1) < 0.001, f"The Correlation of two perfectly collinear arrays should be one, not {corr}"
